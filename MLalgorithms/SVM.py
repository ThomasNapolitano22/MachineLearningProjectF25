...
print("Out of " + str (len(y_test)) + " tests. SVM correctly predicted " + str(amountCorrect) + " of them.")
accuracy = np.sum(y_test == modelPrediction) / modelPrediction.size
print(f"Accuracy of our SVM Model: {accuracy * 100:.2f}%")
...
plt.title("Confusion Matrix of the SVM (Linear) Model")
plt.tight_layout()
plt.savefig("../ModelsandDiagrams/SVMConfusionMatrix.png")
plt.show()
